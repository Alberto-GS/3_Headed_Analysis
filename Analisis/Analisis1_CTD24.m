...
sbe41tempi=interp1(data.sbe41.press(i41_temp),x,CTD24.pres); clear x
[x, i41_sal] = unique(data.sbe41.sal); 
sbe41sali=interp1(data.sbe41.press(i41_sal),data.sbe41.sal,CTD24.pres);clear x
...
[x, i61_temp] = unique(data.sbe61.temp,'rows','stable');
    ...
    try
    sbe61tempi=interp1(data.sbe61.press(i61_temp),x,CTD24.pres); 
    catch ME 
    sbe61tempi=interp1((smooth(data.sbe61.press(i61_temp))),x,CTD24.pres); clear x
    end; clear x
[x, i61_sal] = unique(data.sbe61.sal);
    try
    sbe61sali=interp1(data.sbe61.press(i61_sal),data.sbe61.sal,CTD24.pres);clear x
    catch ME
    sbe61sali=interp1((smooth(data.sbe61.press(i61_sal))),data.sbe61.sal,CTD24.pres);clear x
    end
%RBR
[x, irbr_temp] = unique(data.rbr.temp); 
rbrtempi=interp1(data.rbr.press(irbr_temp),data.rbr.temp,CTD24.pres); clear x
[x, irbr_sal] = unique(data.rbr.sal); 
rbrsali=interp1(data.rbr.press(irbr_sal),data.rbr.sal,CTD24.pres); clear x

%Plot Sensors vs. CTD25 Temp.
figure('units','inch','position',[0,8,12,8]);
subplot(1,2,1); plot(sbe41tempi-CTD24.temp,-CTD24.pres,'LineWidth',1.2);hold on
plot(sbe61tempi-CTD24.temp,-CTD24.pres,'LineWidth',1.2);
plot(rbrtempi(6:end)-CTD24.temp(6:end),-CTD24.pres(6:end),'LineWidth',1.2);
...
subplot(1,2,2); plot(sbe41sali-CTD24.sal,-CTD24.pres,'LineWidth',1.2);hold on
plot(sbe61sali-CTD24.sal,-CTD24.pres,'LineWidth',1.2);
plot(rbrsali(6:end)-CTD24.sal(6:end),-CTD24.pres(6:end),'LineWidth',1.2);
